...
namespace Shard.Shared.Web.IntegrationTests
{
    public partial class BaseIntegrationTests<TEntryPoint, TWebApplicationFactory>
    {
            ...
            JumpingCargo_StandardScenario()
        {
            ...
            var (userPath, unitPath, unit) = await CreateTransportReadyToJump(client);
            ...
            var jumpingResponse = await client.PutAsJsonAsync(unitPath, unit);
            return (jumpingResponse, userEntry, unitEntry);
        }

        private async Task<(string userPath, string unitPath, JObject unit)> CreateTransportReadyToJump(HttpClient client)
        {
            var (userPath, unitPath, unit) = await CreateTransportAndLoadScenario(client);
            unit["destinationSystem"] = "80ad7191-ef3c-14f0-7be8-e875dad4cfa6";
            unit["destinationPlanet"] = null;
            using var response = await client.PutAsJsonAsync(unitPath, unit);
            await response.AssertSuccessStatusCode();
